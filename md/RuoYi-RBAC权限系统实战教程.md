...
- [第七章 用户管理模块实战](#第七章-用户管理模块实战)
  - [7.1 Req/Resp 设计模式](#71-dtovo-设计模式)
...
```
RuoYi/
├── ruoyi-admin/          # 启动与 Web 层模块
│   └── src/main/
│       ├── java/com/ruoyi/admin/
│       │   ├── config/              # 启动模块内配置
│       │   └── web/                 # Controller 入口层
│       │       ├── auth/            # 认证接口
│       │       └── system/          # 用户/角色/菜单/部门接口
│       └── resources/
│           ├── application.yml      # 主配置
│           └── application-dev.yml  # 开发环境配置
│
├── ruoyi-common/         # 公共模块（核心抽象、MyBatis、数据权限、安全、Redis）
...
│       ├── mybatis/                 # MyBatis-Plus 扩展
│       │   ├── handler/MyMetaObjectHandler.java     # 自动填充处理器
│       │   └── interceptor/DataScopeInterceptor.java # 数据权限 SQL 拦截器
│       ├── datascope/               # 数据权限能力
│       │   ├── annotation/DataScope.java            # 数据权限注解
│       │   ├── aspect/DataScopeAspect.java          # 数据权限切面
│       │   ├── context/DataScopeContext.java        # 数据权限上下文
│       │   └── service/DataScopeService.java        # 数据权限服务接口
│       ├── security/                # 认证鉴权组件
...
先执行项目中的 `sql/schema.sql` 创建表结构，再执行 `sql/data.sql` 导入初始数据：

如果你要重建数据库，推荐直接执行下面两条命令：

```bash
mysql --default-character-set=utf8mb4 -u root -p -e "source d:/dev/code/RuoYi/sql/schema.sql"
mysql --default-character-set=utf8mb4 -u root -p -e "source d:/dev/code/RuoYi/sql/data.sql"
```

这里必须显式指定 `--default-character-set=utf8mb4`，因为 `data.sql` 里包含中文初始化数据；如果不带这个参数，命令行导入时可能把字符集问题误报成 `Data too long for column 'dept_name'`。
...
2. 等待 Maven 依赖下载完成
3. 运行 `ruoyi-admin/src/main/java/com/ruoyi/RuoYiAdminApplication.java`
...
Started RuoYiAdminApplication in 3.xxx seconds
```
...
2. 先调用 `POST /auth/login` 获取 token
3. 点击右上角 `Authorize`
4. 按提示填写 `Bearer <token>`
5. 再调试 `/auth/info`、`/system/user/list` 等需要认证的接口

> 现在项目已经配置了 OpenAPI Bearer 安全方案，`ruoyi-admin` 下的管理接口会在文档中标记为需要授权；`/auth/login`、`/auth/register` 这类公开接口不会被误标。
...
| 注册用户 | 普通角色（common） | 基础查看权限（默认查看研发部门数据） |
...
这里数据库列类型依然使用 `DATETIME`，Java 侧统一映射为 `LocalDateTime`，接口序列化后仍然是 `yyyy-MM-dd HH:mm:ss` 字符串。
...
public class SysUser extends BaseEntity {
    ...
    private LocalDateTime beginTime;  // 查询条件用
    @TableField(exist = false)
    private LocalDateTime endTime;    // 查询条件用
}
```
...
public class AuthController {
    ...
    public AuthController(AuthService authService, RuoyiProperties ruoyiProperties) {
        this.authService = authService;
        this.ruoyiProperties = ruoyiProperties;
    }

    @PostMapping("/login")
    public R<LoginResp> login(@Valid @RequestBody LoginReq loginBody,
                            HttpServletRequest request) {
        return R.ok(authService.login(loginBody, request.getRemoteAddr()));
    }
}
```
...
public class TokenService {
    ...
    public boolean validateToken(String token) {
        ...
        // 检查 Redis 中是否存在
        Object cachedToken = redisTemplate.opsForValue().get("login_tokens:" + userId);
    ...
    public boolean removeToken(String userId) {
        return Boolean.TRUE.equals(
            redisTemplate.delete("login_tokens:" + userId)
        );
    }
}
```
...
> **状态码口径**: 当前项目中的认证失败不再只是响应体里的 `code=401`，而是会真实返回 HTTP `401`；普通业务校验默认返回 HTTP `400`，权限不足返回 HTTP `403`。
...
public class SecurityConfig {
    ...
    public CorsConfigurationSource corsConfigurationSource() {
        ...
        configuration.setAllowedOrigins(ruoyiProperties.getCors().getAllowedOrigins());
        ...
        if (token != null) {
            try {
                ...
                if (tokenService.validateToken(token)) {
                    ...
                    permissionService.getPermsByUserId(userIdValue)
                            .forEach(perm -> authorities.add(new SimpleGrantedAuthority(perm)));
                    permissionService.getRoleKeysByUserId(userIdValue)
                            .forEach(roleKey -> authorities.add(new SimpleGrantedAuthority("ROLE_" + roleKey)));
...
@GetMapping("/list")
public R<PageResult<SysUserResp>> list(...) { ... }
...
> **注意**: 现在权限与角色会在 `JwtAuthenticationFilter` 中通过 `PermissionService` 查询后写入 `Authentication.getAuthorities()`，后续 `@PreAuthorize` 会直接基于这些权限集合做判断。
...
这里的 `401 / 403` 不只是响应体中的业务码，HTTP 状态码本身也分别是 `401 / 403`。前端联调时应优先按 HTTP 状态码处理，再读取统一响应体中的 `code` 与 `msg`。
...
│  记录请求开始：GET /system/user/list | IP=127.0.0.1           │
└─────────────────────────────────────────────────────────────┘
        │
        ▼
┌─────────────────────────────────────────────────────────────┐
...
┌─────────────────────────────────────────────────────────────┐
│  @PreAuthorize("hasAuthority('system:user:list')")           │
│  从 Authentication.authorities 读取权限 → 包含后通过          │
└─────────────────────────────────────────────────────────────┘
        │
        ▼
┌─────────────────────────────────────────────────────────────┐
│  SysUserController.list()                                    │
│  执行业务逻辑 → 查询用户列表 → 返回 R<PageResult<SysUserResp>> │
└─────────────────────────────────────────────────────────────┘
        │
        ▼
┌─────────────────────────────────────────────────────────────┐
...
| Req | 请求参数封装（入参） | SysUserCreateReq / SysUserUpdateReq |
| Resp | 响应数据封装（出参） | SysUserResp |
...
本项目采用 **Req/Resp 分离** 模式，不直接暴露数据库实体：

```
前端请求 → Req（入参校验）→ Entity（数据库操作）→ Resp（出参过滤）
```

**SysUserCreateReq / SysUserUpdateReq** — 拆分后的用户请求体：
...
**SysUserResp** — 返回给前端的响应体（过滤掉密码等敏感字段）：
...
public class SysUserResp {
    ...
    @JsonFormat(pattern = "yyyy-MM-dd HH:mm:ss")
    private LocalDateTime createTime;
    private List<SysRoleResp> roles;  // 关联的角色列表
}
```

**SysUserConvert** — 使用 MapStruct 统一完成 Req/Resp 与实体之间的转换：
...
public class SysUserServiceImpl extends ServiceImpl<SysUserMapper, SysUser>
    ...
    public int createUser(SysUserCreateReq req) {
        ...
        if (!save(user)) {
            throw new ServiceException(500, "创建用户失败");
        }
        return 1;
    }
    ...
    public int deleteUserByIds(Long[] userIds) {
        ...
        if (!removeByIds(Arrays.asList(userIds))) {
...
**SysRoleCreateReq / SysRoleUpdateReq** — 角色请求体（包含菜单关联）：
...
private List<MenuTreeResp> buildChildren(Long parentId,
...
├── 深圳总公司 (deptId=101, ancestors="0,100")
│   ├── 研发部门 (deptId=103, ancestors="0,100,101")
│   └── 市场部门 (deptId=104, ancestors="0,100,101")
└── 长沙分公司 (deptId=102, ancestors="0,100")
```
...
演示初始化数据中，普通角色 `common` 默认使用 `data_scope = 2`，并通过 `sys_role_dept` 关联到研发部门（`dept_id = 103`），这样既能演示“自定义数据权限”，又不会出现“菜单能看到但列表查不到数据”的情况。
| 5 | 仅本人数据 | 只看自己创建的数据 |
...
#### 问题 5：401/403 在调试工具里看起来像空 body

**现象**: APIFox、浏览器插件或某些命令行工具里，HTTP 401/403 时看起来没有响应体。

**原因**: 当前项目已经自定义了 `AuthenticationEntryPoint` 和 `AccessDeniedHandler`，后端会返回 JSON；但有些工具在非 2xx 状态下默认不展开或不展示响应体，容易误判成“后端没返回”。
...
联调时优先确认 HTTP 状态码是否分别为 `401 / 403`；如果工具支持查看 raw response，再读取 JSON 响应体中的 `code` 与 `msg`。
...
| 场景 | 请求 Body | 预期响应 |
|------|-----------|----------|
| 密码错误 | `{"username":"admin","password":"wrong"}` | HTTP `401`，响应体 `{"code":401,"msg":"密码错误"}` |
| 用户不存在 | `{"username":"nonexist","password":"123"}` | HTTP `401`，响应体 `{"code":401,"msg":"用户不存在或密码错误"}` |
...
| 菜单 | GET | /system/menu/role-menu-tree-select/{roleId} | system:role:query | 获取角色菜单ID（旧路径兼容 `roleMenuTreeVOselect`） |
